...
defineOptions({
    layout: (h_: typeof h, page: VNode) =>
        h_(AppLayout, { breadcrumbs: [{ title: 'Dashboard', href: '/dashboard' }, { title: 'Hotel Categories', href: '/dashboard/hotel-categories' }, { title: 'Delete', href: '#' }] }, () => page),
});
...
const handleSubmit = () => {
    form.delete(`/dashboard/hotel-categories/${props.category.uuid}`, { onSuccess: () => { close(); redirect(); } });
};
</script>

<template>
    <ModalForm v-model:open="isOpen" title="Delete Category" description="This action cannot be undone" mode="delete" size="md" submit-text="Delete Category" :loading="form.processing" :disabled="!confirmed" @submit="handleSubmit">
        <div class="flex items-start gap-3 rounded-lg border border-destructive/50 bg-destructive/10 p-4">
            ...
            <p class="text-sm font-medium text-destructive">You are about to delete "{{ category.name }}"</p>
        </div>
