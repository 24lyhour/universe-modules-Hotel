    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6">
        ...
        >
            ...
            <template #cell-price="{ item }">
                <div v-if="item.effective_discount_price && item.effective_discount_price < item.price">
                    <span class="font-medium text-green-600">{{ formatCurrency(item.effective_discount_price) }}</span>
                    ...
                    <Badge variant="destructive" class="ml-1 text-[10px] px-1 py-0">
                        -{{ Math.round(((item.price - item.effective_discount_price) / item.price) * 100) }}%
                    </Badge>
                    <p v-if="!item.discount_price" class="text-[10px] text-muted-foreground">via hotel</p>
                </div>
                <div v-else>
