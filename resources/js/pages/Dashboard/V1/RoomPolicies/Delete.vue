...
const handleSubmit = () => {
    form.delete(`/dashboard/hotel-room-policies/${props.policy.uuid}`, {
        onSuccess: () => {
            toast.success('Room policy deleted successfully.');
            setTimeout(() => {
                close();
                redirect();
            }, 100);
...
<template>
    <ModalForm
        v-model:open="isOpen"
        title="Delete Room Policy"
        description="This action cannot be undone"
        mode="delete"
        size="md"
        submit-text="Delete Policy"
            ...
            <div class="flex items-start gap-3 rounded-lg border border-destructive/50 bg-destructive/10 p-4">
                ...
                <div class="space-y-1">
                    <p class="text-sm font-medium text-destructive">
                        You are about to delete this policy
                    </p>
                    <p class="text-sm text-muted-foreground">
                        <strong>{{ policy.title }}</strong> will be moved to trash.
                    </p>
                </div>
            </div>
