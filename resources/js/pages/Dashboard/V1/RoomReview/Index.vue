...
import type { RoomReview, RoomReviewStats, PaginatedResponse } from '@hotel/types';
...
import { RoomReviewStatsWidget, RoomReviewCardWidget } from '@hotel/Components/Dashboard/V1/widgets/RoomReviewWidgets';

defineOptions({
    layout: (h_: typeof h, page: VNode) =>
        h_(AppLayout, { breadcrumbs: [{ title: 'Dashboard', href: '/dashboard' }, { title: 'Hotels', href: '/dashboard/hotels' }, { title: 'Reviews', href: '/dashboard/hotel-room-reviews' }] }, () => page),
});

const props = defineProps<{
    reviews: PaginatedResponse<RoomReview>;
    filters: Record<string, string>;
    stats: RoomReviewStats;
    rooms: { id: number; uuid: string; name: string }[];
}>();

const search = ref(props.filters.search || '');
const isActiveFilter = ref(props.filters.is_active || 'all');
const ratingFilter = ref(props.filters.rating || 'all');
const roomFilter = ref(props.filters.room || 'all');
...
const hasActiveFilters = computed(() => !!(
    ...
    (roomFilter.value && roomFilter.value !== 'all')
));
...
const getFilterParams = () => ({
    ...
    room: roomFilter.value !== 'all' ? roomFilter.value : undefined,
    per_page: perPage.value,
});

const applyFilters = (overrides = {}) => {
    router.get('/dashboard/hotel-room-reviews', { ...getFilterParams(), ...overrides }, { preserveState: true, preserveScroll: true });
};
...
const handleToggleActive = (review: RoomReview, isActive: boolean) => {
    router.patch(`/dashboard/hotel-room-reviews/${review.uuid}/status`, { is_active: isActive }, {
        preserveScroll: true,
        onSuccess: () => toast.success(`Review ${isActive ? 'activated' : 'deactivated'}`),
    });
};

const handleView = (review: RoomReview) => router.visit(`/dashboard/hotel-room-reviews/${review.uuid}`);
const handleReply = (review: RoomReview) => router.visit(`/dashboard/hotel-room-reviews/${review.uuid}`);
const handleDelete = (review: RoomReview) => router.visit(`/dashboard/hotel-room-reviews/${review.uuid}/delete`);

</script>

<template>
    <Head title="Room Reviews" />

    <div class="flex h-full flex-1 flex-col gap-6 p-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold tracking-tight">Room Reviews</h1>
                <p class="text-muted-foreground">Monitor and manage customer feedback across your rooms</p>
            </div>
            <ButtonGroup>
                ...
                <Button variant="outline" size="sm" class="h-9" as-child>
                    <Link href="/dashboard/hotel-room-reviews/trash">
        ...
        <!-- Stats Section -->
        <RoomReviewStatsWidget :stats="stats" />
        ...
        <div class="bg-card border rounded-xl p-4 shadow-sm">
            <div class="flex flex-wrap items-center gap-4">
                <div class="relative flex-1 min-w-[240px]">
                    ...
                    <Input v-model="search" placeholder="Search by guest, comment or room..." class="pl-9 h-10 border-muted-foreground/20 focus-visible:ring-primary" @keyup.enter="applyFilters({ page: 1 })" />
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Select v-model="roomFilter" @update:model-value="applyFilters({ page: 1 })">
                        <SelectTrigger class="w-[180px] h-10 border-muted-foreground/20">
                            <SelectValue placeholder="All Rooms" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="all">All Rooms</SelectItem>
                            <SelectItem v-for="r in rooms" :key="r.id" :value="r.id.toString()">{{ r.name }}</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
