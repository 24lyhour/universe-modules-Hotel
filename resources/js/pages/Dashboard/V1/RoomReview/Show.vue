...
import { ArrowLeft, Star, CheckCircle, XCircle, Clock, MessageSquare, ThumbsUp, BedDouble as RoomIcon } from 'lucide-vue-next';
import { toast } from 'vue-sonner';

defineOptions({
    layout: (h_: typeof h, page: VNode) =>
        h_(AppLayout, { breadcrumbs: [{ title: 'Dashboard', href: '/dashboard' }, { title: 'Reviews', href: '/dashboard/hotel-room-reviews' }, { title: 'Detail', href: '#' }] }, () => page),
});
...
const handleReply = () => {
    replyForm.patch(`/dashboard/hotel-room-reviews/${props.review.uuid}/reply`, {
        onSuccess: () => { showReplyForm.value = false; toast.success('Reply sent.'); },
    });
};

const handleToggleActive = (isActive: boolean) => {
    router.patch(`/dashboard/hotel-room-reviews/${props.review.uuid}/status`, { is_active: isActive }, {
        preserveScroll: true,
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6">
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-6">
                ...
                <!-- Room -->
                <Card v-if="review.room">
                    <CardHeader><CardTitle>Room</CardTitle></CardHeader>
                    <CardContent>
                        <Link :href="`/dashboard/rooms/${review.room.uuid}`" class="flex items-center gap-3 hover:bg-muted/50 rounded-lg p-2 -m-2">
                            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
                                <RoomIcon class="h-5 w-5 text-primary" />
                            </div>
                            <div class="flex flex-col">
                                <span class="font-medium text-sm">{{ review.room.name }}</span>
                                <span v-if="review.room.hotel" class="text-xs text-muted-foreground">{{ review.room.hotel.name }}</span>
                            </div>
                        </Link>
                    </CardContent>
                </Card>
